...
func main() {
	...
	for i, snap := range snaps {
		...
		for j := range hds {
			...
			if err != nil { log.Fatal(err.Error()) }
			err = io.ReadSheetPositionsAt(files[j], vs)
			if err != nil { log.Fatal(err.Error()) }

			bin(xs, vs, ns, rBins, vBins, s, v, vLim)
		}
		writeBins(ns, rBins, vBins)
	}
}
...
func bin(xs, vs []rgeom.Vec, ns []int, rBins, vBins int,
	...
	for i := range xs {
		...
		if ri > rBins || vi > vBins { continue }
		ns[ri + vi * rBins]++
	}
}

func writeBins(ns []int, rBins, vBins int) error {
	fname := fmt.Sprintf("s%d_id%d.dat")
