...
for (i, host) in enumerate(hosts):
    ...
    sub_counts.append(n)
    r_lo, r_hi = 0, 2
    if n > SUBHALO_LIM and LOW_GAMMA < host.gamma < HIGH_GAMMA:
        bins = 15

        vals, edges = np.histogram(
            (ds / host.r_sp * r_sp_scale)[mask], bins=bins, range=(r_lo, r_hi),
        )
        norms, _ = np.histogram(
            (ds / host.r_sp * r_sp_scale), bins=bins, range=(r_lo, r_hi),
        )
        norms[norms <= 0] = 1
        b_r_sp.append(1 - vals / norms)

        vals, _ = np.histogram(
            (ds / host.r_max*r_max_scale)[mask], bins=bins, range=(r_lo, r_hi),
        )
        norms, _ = np.histogram(
            (ds / host.r_max * r_max_scale), bins=bins, range=(r_lo, r_hi),
        )
        norms[norms <= 0] = 1
        b_r_max.append(1 - vals / norms)

        vals, _ = np.histogram(
            (ds / host.r200m*r200m_scale)[mask], bins=bins, range=(r_lo, r_hi),
        )
        norms, _ = np.histogram(
            (ds / host.r200m * r200m_scale), bins=bins, range=(r_lo, r_hi),
        )
        norms[norms <= 0] = 1
        b_r200m.append(1 - vals / norms)

        vals, _ = np.histogram(
            (ds / host.r200c*r200c_scale)[mask], bins=bins, range=(r_lo, r_hi),
        )
        norms, _ = np.histogram(
            (ds / host.r200c * r200c_scale), bins=bins, range=(r_lo, r_hi),
        )
        norms[norms <= 0] = 1
        b_r200c.append(1 - vals / norms)
